const translations = {
    vi: {
        common: {
            ok: "OK",
            cancel: "Hủy",
            close: "Đóng",
        },
        login: {
            title: "Đăng nhập BurgerFast",
            emailPlaceholder: "Nhập email",
            passwordPlaceholder: "Nhập mật khẩu",
            loginButton: "Đăng nhập",
            forgot: "Quên mật khẩu?",
            noAccount: "Bạn chưa có tài khoản?",
            createAccount: "Tạo tài khoản",
            errors: {
                requiredEmail: "Vui lòng nhập email",
                invalidEmail: "Email phải đúng định dạng (ví dụ: abc@gmail.com)",
                requiredPassword: "Vui lòng nhập mật khẩu",
                passwordMin: "Mật khẩu tối thiểu 6 ký tự",
                invalidCredentials: "Sai email hoặc mật khẩu",
                userNotFound: "Tài khoản không tồn tại",
                unknownError: "Có lỗi xảy ra",
                resetNoticeTitle: "Kiểm tra email",
                resetNoticeMessage: "Nếu email hợp lệ, chúng tôi sẽ gửi liên kết đặt lại mật khẩu. Vui lòng kiểm tra hộp thư (bao gồm thư rác).",
                resetUserNotFound: "Email này chưa được đăng ký",
            },
        },
        register: {
            title: "Tạo tài khoản BurgerFast 🍔",
            haveAccount: "Đã có tài khoản? Đăng nhập",
            registerButton: "Đăng ký",
            labels: {
                name: "Họ và tên",
                dob: "Ngày sinh",
                gender: "Giới tính",
                phone: "Số điện thoại",
                email: "Email",
                password: "Mật khẩu",
                confirm: "Xác nhận mật khẩu",
                agreeText: "Tôi đồng ý với {terms} và {privacy}",
                terms: "Điều khoản, Điều kiện",
                privacy: "Chính sách bảo mật",
            },
            genderOptions: {
                male: "Nam",
                female: "Nữ",
                other: "Khác",
            },
            errors: {
                required: "{field} bắt buộc nhập",
                invalidPhone: "Số điện thoại không hợp lệ (chỉ gồm 9-12 chữ số)",
                invalidEmail: "Email phải đúng định dạng (ví dụ: abc@gmail.com)",
                passwordMin: "Mật khẩu tối thiểu 6 ký tự",
                confirmMismatch: "Mật khẩu xác nhận không khớp",
                agreeRequired: "Bạn phải đồng ý điều khoản sử dụng",
                registerFailedTitle: "Lỗi đăng ký",
                registerFailedMessage: "Có lỗi xảy ra",
                emailExists: "Email này đã tồn tại",
                weakPassword: "Mật khẩu quá yếu (ít nhất 6 ký tự)",
            },
            alerts: {
                successTitle: "🎉 Thành công",
                successMessage: "Tạo tài khoản thành công!",
            },
        },
        guestAccount: {
            login: "Đăng nhập",
            register: "Đăng ký",
            trackOrders: "Theo dõi đơn hàng",
            language: "Ngôn ngữ",
            home: "Trang chủ",
            menu: "Menu",
            account: "Tài khoản",
            continueAsGuest: "Tiếp tục với tư cách khách",
        },
        customerAccount: {
            myProfile: "Hồ sơ của tôi",
            trackOrders: "Theo dõi đơn hàng",
            language: "Ngôn ngữ",
            tiers: {
                new: "Thành viên Mới",
                bronze: "Thành viên Đồng",
                silver: "Thành viên Bạc",
                gold: "Thành viên Vàng",
            },
            changePassword: "Đổi mật khẩu",
            logout: "Đăng xuất",
            deleteAccount: "Xóa tài khoản",
            points: "điểm",
            alerts: {
                logout: {
                    title: "Đăng xuất",
                    message: "Bạn đã đăng xuất thành công!",
                },
                delete: {
                    title: "Xóa tài khoản",
                    message: "Bạn có chắc muốn xóa tài khoản này không?",
                    cancel: "Hủy",
                    confirm: "Xóa",
                    successTitle: "Đã xóa",
                    successMessage: "Tài khoản của bạn đã được xóa.",
                    errorTitle: "Lỗi",
                    errorMessage: "Không thể xóa tài khoản. Vui lòng đăng nhập lại và thử lại.",
                },
            },
        },
        changePassword: {
            title: "Đổi mật khẩu",
            currentPlaceholder: "Mật khẩu hiện tại",
            newPlaceholder: "Mật khẩu mới",
            confirmPlaceholder: "Xác nhận mật khẩu mới",
            updateButton: "Cập nhật mật khẩu",
            errors: {
                fillAll: "Vui lòng nhập đầy đủ thông tin.",
                mismatch: "Mật khẩu mới và xác nhận mật khẩu không khớp.",
                wrongCurrent: "Mật khẩu hiện tại không đúng.",
                minLength: "Mật khẩu mới phải có ít nhất 6 ký tự.",
                updateFailed: "Đổi mật khẩu thất bại. Vui lòng thử lại.",
                success: "✅ Mật khẩu đã được thay đổi thành công!",
            },
        },
        profile: {
            headerTitle: "Hồ sơ cá nhân",
            loading: "Đang tải thông tin...",
            roleCustomer: "Khách hàng",
            sectionTitle: "Thông tin cá nhân",
            labels: {
                name: "Họ và tên:",
                email: "Email:",
                phone: "Số điện thoại:",
                gender: "Giới tính:",
                dob: "Ngày sinh:",
                createdAt: "Ngày tạo tài khoản:",
                points: "Điểm tích lũy:",
            },
            notUpdated: "Chưa cập nhật",
        },
        orders: {
            title: "Theo dõi đơn hàng",
            loading: "Đang tải đơn hàng...",
            emptyText: "Bạn chưa có đơn hàng nào.",
            orderId: "Mã đơn:",
            orderDate: "Ngày đặt:",
            total: "Tổng tiền:",
            status: "Trạng thái:",
            // UI search strings (design-only)
            searchPlaceholder: "Số điện thoại hoặc email",
            searchButton: "Tìm",
            searchHint: "Tìm đơn hàng bằng số điện thoại hoặc email (thiết kế tạm thời)",
            notLoggedInTitle: "Bạn chưa đăng nhập",
            notLoggedInSubtitle: "Hãy đăng nhập để xem các đơn hàng của bạn.",
            loginNow: "Đăng nhập ngay",
        },
        languages: {
            vi: "Tiếng Việt",
            en: "English",
            zh: "中文",
        },
        selectRole: {
            welcome: "Chào mừng bạn đến với BurgerFast!",
            loginButton: "Đăng nhập vào tài khoản",
            continueGuest: "Tiếp tục với tư cách khách",
        },
    },
    en: {
        common: {
            ok: "OK",
            cancel: "Cancel",
            close: "Close",
        },
        login: {
            title: "Login BurgerFast",
            emailPlaceholder: "Enter email",
            passwordPlaceholder: "Enter password",
            loginButton: "Login",
            forgot: "Forgot password?",
            noAccount: "Don't have an account?",
            createAccount: "Create account",
            errors: {
                requiredEmail: "Please enter email",
                invalidEmail: "Email must be valid (e.g., abc@gmail.com)",
                requiredPassword: "Please enter password",
                passwordMin: "Password must be at least 6 characters",
                invalidCredentials: "Incorrect email or password",
                userNotFound: "Account does not exist",
                unknownError: "An error occurred",
                resetNoticeTitle: "Check your email",
                resetNoticeMessage: "If the email is valid, we will send a password reset link. Please check your inbox (including spam).",
                resetUserNotFound: "This email is not registered",
            },
        },
        register: {
            title: "Create a BurgerFast account 🍔",
            haveAccount: "Already have an account? Login",
            registerButton: "Register",
            labels: {
                name: "Full name",
                dob: "Date of birth",
                gender: "Gender",
                phone: "Phone",
                email: "Email",
                password: "Password",
                confirm: "Confirm password",
                agreeText: "I agree to the {terms} and {privacy}",
                terms: "Terms & Conditions",
                privacy: "Privacy Policy",
            },
            genderOptions: {
                male: "Male",
                female: "Female",
                other: "Other",
            },
            errors: {
                required: "{field} is required",
                invalidPhone: "Phone number is invalid (9-12 digits)",
                invalidEmail: "Email must be valid (e.g., abc@gmail.com)",
                passwordMin: "Password must be at least 6 characters",
                confirmMismatch: "Password confirmation does not match",
                agreeRequired: "You must agree to the terms",
                registerFailedTitle: "Registration error",
                registerFailedMessage: "An error occurred",
                emailExists: "This email is already in use",
                weakPassword: "Password is too weak (at least 6 characters)",
            },
            alerts: {
                successTitle: "🎉 Success",
                successMessage: "Account created successfully!",
            },
        },
        guestAccount: {
            login: "Login",
            register: "Register",
            trackOrders: "Track orders",
            language: "Language",
            home: "Home",
            menu: "Menu",
            account: "Account",
            continueAsGuest: "Continue as Guest",
        },
        customerAccount: {
            myProfile: "My profile",
            trackOrders: "Track orders",
            language: "Language",
            tiers: {
                new: "New member",
                bronze: "Bronze member",
                silver: "Silver member",
                gold: "Gold member",
            },
            changePassword: "Change password",
            logout: "Logout",
            deleteAccount: "Delete account",
            points: "points",
            alerts: {
                logout: {
                    title: "Logout",
                    message: "You have been logged out successfully!",
                },
                delete: {
                    title: "Delete account",
                    message: "Are you sure you want to delete this account?",
                    cancel: "Cancel",
                    confirm: "Delete",
                    successTitle: "Deleted",
                    successMessage: "Your account has been deleted.",
                    errorTitle: "Error",
                    errorMessage: "Could not delete account. Please sign in again and try.",
                },
            },
        },
        changePassword: {
            title: "Change password",
            currentPlaceholder: "Current password",
            newPlaceholder: "New password",
            confirmPlaceholder: "Confirm new password",
            updateButton: "Update password",
            errors: {
                fillAll: "Please fill in all fields.",
                mismatch: "New password and confirmation do not match.",
                wrongCurrent: "Current password is incorrect.",
                minLength: "New password must be at least 6 characters.",
                updateFailed: "Failed to update password. Please try again.",
                success: "✅ Password changed successfully!",
            },
        },
        profile: {
            headerTitle: "Profile",
            loading: "Loading...",
            roleCustomer: "Customer",
            sectionTitle: "Personal information",
            labels: {
                name: "Full name:",
                email: "Email:",
                phone: "Phone:",
                gender: "Gender:",
                dob: "Date of birth:",
                createdAt: "Account created:",
                points: "Accumulated points:",
            },
            notUpdated: "Not updated",
        },
        orders: {
            title: "Track orders",
            loading: "Loading orders...",
            emptyText: "You have no orders yet.",
            orderId: "Order ID:",
            orderDate: "Order date:",
            total: "Total:",
            status: "Status:",
            // UI search strings (design-only)
            searchPlaceholder: "Phone number or email",
            searchButton: "Search",
            searchHint: "Search orders by phone or email (design only)",
            notLoggedInTitle: "You're not signed in",
            notLoggedInSubtitle: "Sign in to view your orders.",
            loginNow: "Sign in",
        },
        languages: {
            vi: "Tiếng Việt",
            en: "English",
            zh: "中文",
        },
        selectRole: {
            welcome: "Welcome to the BurgerFast!",
            loginButton: "Login to your account",
            continueGuest: "Continue as Guest",
        },
    },
    zh: {
        common: {
            ok: "确定",
            cancel: "取消",
            close: "关闭",
        },
        login: {
            title: "登录 BurgerFast",
            emailPlaceholder: "输入邮箱",
            passwordPlaceholder: "输入密码",
            loginButton: "登录",
            forgot: "忘记密码?",
            noAccount: "还没有账号?",
            createAccount: "创建账号",
            errors: {
                requiredEmail: "请输入邮箱",
                invalidEmail: "邮箱格式不正确（例如：abc@gmail.com）",
                requiredPassword: "请输入密码",
                passwordMin: "密码至少需要6个字符",
                invalidCredentials: "邮箱或密码不正确",
                userNotFound: "账号不存在",
                unknownError: "发生错误",
                resetNoticeTitle: "检查邮箱",
                resetNoticeMessage: "如果邮箱有效，我们会发送重置密码的链接。请检查收件箱（包括垃圾邮件）。",
                resetUserNotFound: "该邮箱尚未注册",
            },
        },
        register: {
            title: "创建 BurgerFast 账号 🍔",
            haveAccount: "已有账号? 登录",
            registerButton: "注册",
            labels: {
                name: "姓名",
                dob: "出生日期",
                gender: "性别",
                phone: "电话",
                email: "邮箱",
                password: "密码",
                confirm: "确认密码",
                agreeText: "我同意{terms}和{privacy}",
                terms: "条款与条件",
                privacy: "隐私政策",
            },
            genderOptions: {
                male: "男",
                female: "女",
                other: "其他",
            },
            errors: {
                required: "{field}为必填项",
                invalidPhone: "电话号码无效（9-12位数字）",
                invalidEmail: "邮箱格式不正确（例如：abc@gmail.com）",
                passwordMin: "密码至少需要6个字符",
                confirmMismatch: "确认密码不匹配",
                agreeRequired: "您必须同意条款",
                registerFailedTitle: "注册失败",
                registerFailedMessage: "发生错误",
                emailExists: "该邮箱已被占用",
                weakPassword: "密码过于简单（至少6个字符）",
            },
            alerts: {
                successTitle: "🎉 成功",
                successMessage: "账号创建成功！",
            },
        },
        guestAccount: {
            login: "登录",
            register: "注册",
            trackOrders: "查看订单",
            language: "语言",
            home: "首页",
            menu: "菜单",
            account: "账户",
            continueAsGuest: "以游客身份继续",
        },
        customerAccount: {
            myProfile: "我的资料",
            trackOrders: "查看订单",
            language: "语言",
            tiers: {
                new: "新会员",
                bronze: "铜牌会员",
                silver: "银牌会员",
                gold: "金牌会员",
            },
            changePassword: "修改密码",
            logout: "登出",
            deleteAccount: "删除账号",
            points: "积分",
            alerts: {
                logout: {
                    title: "登出",
                    message: "您已成功登出！",
                },
                delete: {
                    title: "删除账号",
                    message: "您确定要删除此账号吗？",
                    cancel: "取消",
                    confirm: "删除",
                    successTitle: "已删除",
                    successMessage: "您的账号已被删除。",
                    errorTitle: "错误",
                    errorMessage: "无法删除账号。请重新登录并重试。",
                },
            },
        },
        changePassword: {
            title: "修改密码",
            currentPlaceholder: "当前密码",
            newPlaceholder: "新密码",
            confirmPlaceholder: "确认新密码",
            updateButton: "更新密码",
            errors: {
                fillAll: "请填写所有字段。",
                mismatch: "新密码与确认密码不匹配。",
                wrongCurrent: "当前密码不正确。",
                minLength: "新密码至少需要6个字符。",
                updateFailed: "更新密码失败，请重试。",
                success: "✅ 密码已成功更改！",
            },
        },
        profile: {
            headerTitle: "个人资料",
            loading: "正在加载信息...",
            roleCustomer: "客户",
            sectionTitle: "个人信息",
            labels: {
                name: "姓名:",
                email: "邮箱:",
                phone: "电话:",
                gender: "性别:",
                dob: "出生日期:",
                createdAt: "创建于:",
                points: "积分:",
            },
            notUpdated: "未更新",
        },
        orders: {
            title: "查看订单",
            loading: "正在加载订单...",
            emptyText: "您还没有订单。",
            orderId: "订单号:",
            orderDate: "下单日期:",
            total: "总计:",
            status: "状态:",
            // UI search strings (design-only)
            searchPlaceholder: "手机号或邮箱",
            searchButton: "搜索",
            searchHint: "通过手机号或邮箱搜索订单（仅设计）",
            notLoggedInTitle: "您尚未登录",
            notLoggedInSubtitle: "请登录以查看您的订单。",
            loginNow: "立即登录",
        },
        selectRole: {
            welcome: "欢迎使用 BurgerFast!",
            loginButton: "登录到您的账号",
            continueGuest: "以游客身份继续",
        },
        languages: {
            vi: "越南语",
            en: "英语",
            zh: "中文",
        },
    },
};

export default translations;
